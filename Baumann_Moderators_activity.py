...
def Connect_Infl(N, m, G, G_num, T, beta, Phi, num_infl, m_infl, actarr, activity='False'):
    ...
    for i in range (N+num_infl):
        ...
        if rand <= G[i][T]:
            ...
            if activity == 'True':
                if i >= N:
                    # Pick random people, since moderators sjhouldn´t be strongly influenced by homophily
                    picks = np.random.choice(G_num[:N], m_infl, replace=False, p=actarr)
                else:
                    prob = Probability_Distribution_Opinion(G, G_num[i], T, beta, Phi)
                    picks = np.random.choice(G_num, m, replace=False, p=prob)
...
def Opinion_Dynamics_Infl(N, T, m, K, alpha, beta, gamma, Phi, eps1, eps2, runtime_net, runtime_op, dt, filename, path, num_infl, m_infl, infl_strength, activity='False'):
    ...
    # Create normalized array of activites
    actarr = save[0][:N]
    norm = 0
    for i in range (N):
        norm += actarr[i]
    ...
    while iteration_net < runtime_net:
        ...
        A, acts[iteration_net] = Connect_Infl(N, m, G, G_num, T, beta, Phi, num_infl, m_infl, actarr, activity)
...
path = f"D:\Daten mit Änderungen\Physik\Bachelorarbeit\Generated_Data\Moderators"
...
for i in range (len(alphas)):
    ...
    for j in range (len(cosd)):
        File_Names3 = []
        for k in range (3):
            File_Names3.append(f'Influencers_a{alphas[i]:.1f}_b{beta}_cosd{cosd[j]:.2f}_m{m_infl}_N{num_infl}_str{infl_strength}_act_{k+1}')
...
for i in range (len(alphas)):
    for j in range (len(cosd)):
        ...
        for k in range(3):
            ...
            Opinion_Dynamics_Infl(N, T, m, K, alphas[i], beta, gamma, Phi, eps1, eps2, runtime_net, runtime_op, step, File_Names[i][j][k], path, num_infl, m_infl, infl_strength, activity='True')